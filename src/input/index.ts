...
export function startInputListener(instance: XTouchControl) {
    ...
    instance.input.on('message', (deltaTime: number, message: number[]) => {
        ...
        if (message[0] === 144 && message[1] in ControlMap) {
            ...
            if (message[2] === 127) {
                ...
                action = 'button_' + message[2].toString();
            }
        ...
        } else if (eventsMessage in Events) {
            // Mapped Events
            const event = Events[eventsMessage];
            instance.emit(...(event as [string, ...unknown[]]));
        ...
        } else if (224 <= message[0] && message[0] <= 232) {
            // Fader Moving
